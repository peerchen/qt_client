...
int CProcessInterfaceMS::OnPacket(char * pData, size_t nSize)
{
	...
	CRLog(E_PROINFO,"MS [%s:%d] %s",sPeerIp.c_str(),nPeerPort,GessPacket.Print().c_str());
...
int CProcessInterfaceMS::SendPacket(CPacket & SendGessPacket)
{
	try
	{
		CIpcPacket & pkt = dynamic_cast<CIpcPacket&>(SendGessPacket);
		CRLog(E_PROINFO,"MS Send:%s",pkt.Print().c_str());
