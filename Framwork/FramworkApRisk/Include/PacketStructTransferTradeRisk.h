...
public:
	static void HeadReq2Rsp(HEADER_REQ& stHeadReq,HEADER_RSP& stHeadRsp)
	{
		...
		strcpy(stHeadRsp.session_id, stHeadReq.session_id);
		strcpy(stHeadRsp.rsp_code,"00000000");
		stHeadRsp.head_type = stHeadReq.head_type;
	}
	...
	static int Packet2Struct(CustSpecRsp& stBody, CTradePacket& pkt)
	{
		...
		pkt.GetParameterVal("curr_offset" , stBody.curr_offset);
		return 0;
	}
	...
	static int Struct2Packet(CustSpecRsp& stBody, CTradePacket& pkt)
	{
		...
		pkt.AddParameter("curr_offset" , stBody.curr_offset);
		return 0;
	}
...
*/
	...
	//----------------------报文接口 [3401] 客户浮动盈亏查询 报文结构相关 解析
	static int Packet2Struct(CustBalanceQryReq &stBody, CTradePacket &pkt)
	{
		pkt.GetParameterVal("oper_flag", stBody.oper_flag);
		pkt.GetParameterVal("acct_no"  , stBody.acct_no);
		pkt.GetParameterVal("qry_type" , stBody.qry_type);

		return 0;
	}
	static int Struct2Packet(CustBalanceQryRsp &stBody, CTradePacket &pkt)
	{
		pkt.AddParameter("oper_flag", stBody.oper_flag);
		pkt.AddParameter("acct_no"  , stBody.acct_no);
		pkt.AddParameter("qry_type" , stBody.qry_type);
		pkt.AddParameter("banlance" , stBody.balance);

		return 0;
	}

	static int Packet2Struct(BranchPosiQryReq &stBody, CTradePacket &pkt)
	{
		pkt.GetParameterVal("oper_flag", stBody.oper_flag);
		pkt.GetParameterVal("branch_id", stBody.branch_id);
		pkt.GetParameterVal("prod_code", stBody.prod_code);
	...
	static int Struct2Packet(BranchPosiQryRsp &stBody, CTradePacket &pkt)
	{
		pkt.AddParameter("oper_flag", stBody.oper_flag);
		pkt.AddParameter("alm_result", stBody.alm_result);
		return 0;
