...
typedef struct tagAutoGenFCOrderBatchReq
{
	...
	string       teller_id            ; //监控员 
	ArrayListMsg posi_result          ; //合约、买卖方向、价格、数量
	ArrayListMsg cov_price_info		  ; //平仓价格信息    合约代码、多仓平仓价格、空仓平仓价格
	char         risk_target          ; //强平目标，0：正常；1：追保；2：相对强平
...
//-------------------------------------------------报文接口 [3401]定义
// 接口定义：客户浮动盈亏查询请求报文体
typedef struct tagCustBalanceQryReq
{
	int         oper_flag;   //操作标志    1：查询
	string      acct_no;     //客户号   必填，不填返回空
	string      qry_type;    //1：浮动盈亏；

	tagCustBalanceQryReq()
	{
		oper_flag = 0;
		acct_no = "";
		qry_type = "";
	}
}CustBalanceQryReq, *PCustBlanceQryReq;

typedef struct tagCustBalanceQryRsp
{
	int         oper_flag;   //操作标志    1：查询
	string      acct_no;     //客户号   必填
	string      qry_type;    //1：浮动盈亏；
	string      balance;    //浮动盈亏

	tagCustBalanceQryRsp()
	{
		oper_flag = 0;
		acct_no = "";
		qry_type = "";
...
}CustBalanceQryRsp, *PCustBalanceQryRsp;

typedef struct tagBranchPosiQryReq
{
	int     oper_flag;//oper_flag为1时，查询所有二级机构+branch_id的下级机构，
	//oper_flag为2时，查询指定机构branch_id
	string  branch_id;
	string  prod_code;
}BranchPosiQryReq;

typedef struct tagBranchPosiQryRsp
{
	int            oper_flag;
	ArrayListMsg   alm_result;  //字段：机构编号，机构名称，机构级别，合约代码，多仓数量，空仓数量
}BranchPosiQryRsp;
