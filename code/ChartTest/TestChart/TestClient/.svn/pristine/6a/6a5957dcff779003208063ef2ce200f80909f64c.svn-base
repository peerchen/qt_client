...
#define RT_LOGIN_HJ						0x0110	/* 客户端登录外盘服务器	mod by Ben			*/
...
{	
	...
	static int GetStructSize(CodeInfo* pCode,int nAdd = 0)
	{
		...
		case (HSMarketDataType)FUTURES_MARKET:			 // 期货
		case (HSMarketDataType)WP_MARKET:                // modified by Ben for fix bug that Gold report table do not has data
		...
		default:
			return 0;//return sizeof(sizeof(HSWHRealTime) + nAdd);  // modified by Ben 20110223 handle the problem that application hanging up casually
			break;
		}
...
struct ReqData_Weihu
{
	...
	unsigned long		m_lBeginDate;  // 开始时间
	unsigned long		m_lEndDate;    // 终止时间
...
struct UpData_Weihu
{
	...
	union
	{
		char			m_cCode[6];		   // 证券代码,当m_cCodeType != 0 时有效
