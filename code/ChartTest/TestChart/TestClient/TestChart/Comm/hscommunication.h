...
#define RT_LOGIN_SY   					0x0112  /* 客户端登录石油服务器   			*/

//kenny  2014-6-13
#define RT_REGISTER   					0x0121  /* 注册帐号    			*/
#define RT_QUERY_CLASS   				0x0122  /* 查询权限    			*/
#define RT_QUERY_ACCOUNT   				0x0123  /* 查询验证账户信息  		*/
#define RT_ONLINE_PAY   				0x0124  /* 提交支付单  		*/

//kenny  2015-8-24
#define RT_Request_FixPrice             0x0F02 // 定盘价查询
...
#define Market_SY_MARKET			    0x0021   // 黄金   // added by kenny
...
// -- 1、申请新账号
//客户端请求
struct ReqRegisterUser
{
	char			m_szUser[64];	    // 用户名
	char			m_szPWD[64];	    // 密码(加密密文），详细参加协议
	char			m_szPhone[11];	    // 用户注册手机号
	char            m_checkCode[6];     // 验证码6位数字
};

//服务端返回
struct RspRegisterUser
{
	char            m_result;           // 0 失败， 1 成功
	long            m_strLength;        // m_str 的长度
	char            m_str[1];           // 如果失败，该值为失败原因信息文本
};
...
//kenny  2014-7-8
/*
请求类型: RT_QUERY_ACCOUNT 
功能说明: 返回注册
备	  注: 
*/
/* 请求结构 : 无请求*/
/* 返回结构 */
struct AnsAccountPermission
{
	DataHead		m_dhHead;		//数据报头
	...
	char			m_szUser[32];	// 用户名
	char			m_szPWD[32];	// 密码(加密密文），详细参加协议
    long            m_lDate;        //到期日
	
	char            m_result;       // 0 失败， 1 成功
	long			m_nSize;		//若失败，指明失败原因的内容长度	
	char			m_cData[1];		//失败原因内容
...
struct AnsFixPrice
{
	...
	short 				m_nAlignment;
	long				m_nSize;		//长度	
	FixPriceResult		m_FixData[1];   //公告内容
