...
static int MakeWPIndex(HSMarketDataType x)
{
	...
	//return ( (MakeMainMarket(x) >= (WP_MARKET | WP_INDEX_RJ)) && (MakeMainMarket(x) <= (WP_MARKET | WP_INDEX_YFTMIB)) );
}

static int MakeWP_LME(HSMarketDataType x)
{
	return ( (MakeMainMarket(x) >= (WP_MARKET | WP_LME))  && (MakeMainMarket(x) <= (WP_MARKET | WP_LME_CW)) );
}
...
struct StockUserInfo
{
	...
	{
		CString str = CString(m_cStockName).Left(STOCK_NAME_SIZE);
		str.Trim();
		return str;
	}
	//取得拼音简称
