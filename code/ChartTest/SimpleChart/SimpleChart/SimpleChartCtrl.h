...
protected:
	...
	void SetHJPeriods(DOUBLE dBegin1, DOUBLE dEnd1, DOUBLE dBegin2, DOUBLE dEnd2, DOUBLE dBegin3, DOUBLE dEnd3, DOUBLE dBegin4, DOUBLE dEnd4);//设置X轴的刻度区间
...
private:
	...
	void DrawStepLine(CRect rectBound, int nPos, double dValue,int nFloatPoint=0,CString sValue=_T(""));
...
private: 
	...
	CMemCore* g_CoreClosePrice;	    // 收盘价 
// 	CMemCore* g_CoreOpenPrice;		// 开盘价 
// 	CMemCore* g_CoreHighPrice;		// 最高价 
// 	CMemCore* g_CoreLowPrice;		// 最低价 
// 	CMemCore* g_CoreChangePrice;	// 涨跌  -- 分时图用
//	CMemCore* g_CoreChangeRate;	// 涨跌率 
	CMemCore* g_CoreContractVolume;// 交易量 
//	CMemCore* g_CoreContractAmount;	// 交易额 
	...
	//KENNY 20171010 for 黄金
	double	m_dBegin1;			// 上午开盘时间	
	double  m_dEnd1;			// 上午停盘时间
	double  m_dBegin2;			// 下午开盘时间
	double  m_dEnd2;			// 下午收盘时间
	double	m_dBegin3;			// 上午开盘时间	
	double  m_dEnd3;			// 上午停盘时间
	double  m_dBegin4;			// 下午开盘时间
	double  m_dEnd4;			// 下午收盘时间
	int     m_nPoint1;
	int     m_nPoint2;
	int     m_nPoint3;
	int     m_nPoint4;
	...
	int    m_nDecimalDepressVol;// 分时图交易量的值的浮点数
	CLabelWnd m_LableWndX;
	CLabelWnd m_LableWndYLeft;
	CLabelWnd m_LableWndYRight;
	CPen*	m_pTipPen;
	HWND m_hParentWnd;
public:
