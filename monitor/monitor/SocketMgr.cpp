...
bool CSocketMgr::Init(CResource *res)
{
	...
	for (i=0;i<res->g_vDrebLinkInfo.size();i++)
	{
		...
		if (i == 0)
		{
			tmpsocket->m_nType = MONI_SOCK_H1;
		}
		else
		{
			tmpsocket->m_nType = MONI_SOCK_H2;
		}
	...
	int allszie= 1+drebnum+res->g_nHeartPoolSize+1;
	for (i=1+drebnum;i<allszie;i++)
	{
		CSocketInfo *tmpsocket = new CSocketInfo();
		if (tmpsocket == NULL)
		{
