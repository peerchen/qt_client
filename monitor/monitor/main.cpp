...
// 
// 	CKvData kv;
// 	kv.FromBuffer("69966151119231131 admin                                         #oper_flag=1#host_id=22#node_id=1103#");
// 	std::string ss;
// 	kv.getHead("exch_code",ss);
...
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\monitor",g_pMoniRun.g_pRes.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"monitor_%s",md5class.ToString().c_str());
#else	
	sprintf(shmname,"%s/monitor",g_pMoniRun.g_pRes.g_sCurPath);
#endif
	...
	if ( (ret=fork()) == 0) 
	{
		...
		printf("启动主机资源monitor成功\n");
		g_pExitFlag[0] = 1;
		while (true)
		{
			if (g_pExitFlag[0] != 1)
			{
				printf("------begin exit monitor.......\n");
				g_pMoniRun.Stop();
				printf("------exit monitor ok !\n");
				SLEEP(100);
				g_pExitFlag = NULL;
				return 0;
			}
