...
#define MAX_OFFERINGDATA_LEN     65535   //报盘机最大的数据长度
#define BEATHEARTTICK  5  //心跳间隔

// #define SOCK_UNKNOW   0      //未知
// #define SOCK_CLI      2      //客户端  即交易服务器
// #define SOCK_LISTEN	  7      //侦听端口

#define SOCK_STATUS_UNLOGIN    0    //未登录
#define SOCK_STATUS_LOGIN      1    //正在登录
#define SOCK_STATUS_NORMAL     2    //正常，可以发回报等等
...
public:
	...
	// 函数名: ResetData
	// 编程  : 王明松 2016-4-13 10:50:40
	// 返回  : bool 
	// 参数  : S_OFFERING_DATA &data
	// 描述  : 申请分配空间
	bool ResetData(S_OFFERING_DATA &data);

	// 函数名: SendMsg
	// 编程  : 王明松 2016-4-11 13:47:46
	// 返回  : int 
	// 参数  : S_OFFERING_DATA msg
	// 参数  : int sendflag
	// 描述  : 发送数据，sendflag为0表示正常的发送，1表示消息线程的发送，首先判断m_bIsBp标志，当m_bIsBp为true时表示正在断点，直接放入队列2,若无断点时，只要队列有数据，也放到队列2
	int SendMsg(S_OFFERING_DATA msg,int sendflag);
	...
	/// 参数  : S_BF_RSMSG *msg
	/// 描述  : 获取接收到的数据
	int GetRecvData(S_OFFERING_DATA *msg);
...
//socket公共参数
	...
	int           m_nBpFlag;                  //是否发送断点 //断点标志，当连接连上来并且发送断点后，置断点状态为1 ，取完断点后状态为0
