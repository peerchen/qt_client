...
//转义，在符号','、'\'、'['、']'、'{'、'}'前面增加'\'
UTIL_API std::string addTransStr(std::string sString);
//转义， 将'\,'、'\\'、'\['、'\]'、'\{'、'\}'转为','、'\'、'['、']'、'{'、'}'
UTIL_API std::string delTransStr(std::string sString);
//去除最外层括号'['']'，如果最外层不是括号，返回-1
UTIL_API int stripBracket(string &sString);
//去除最外层花括号'{''}'，如果最外层不是花括号，返回-1
UTIL_API int stripBrace(string &sString);
...
UTIL_API std::vector< std::string > explodeGTP(char cSeparator,const std::string &sString);
UTIL_API string::size_type findFirstOf(char cSeparator,const std::string &sString);
