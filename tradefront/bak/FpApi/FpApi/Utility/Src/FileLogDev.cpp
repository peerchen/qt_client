...
int CFileLogDev::Initial(CConfig * pCfg)
{
	...
	{
		//日志文件最大尺寸未配置，采用默认值
		cout << "The maximum size of the log file is not configured, with default values " << FILELOG_MAX_SIZE << "KB" << endl;
		m_nMaxSize = FILELOG_MAX_SIZE * 1024; // 以K字节为单位
	}
	else
	{
		//日志文件最大尺寸设置为
		cout << "The maximum log file size is set to " << m_nMaxSize << "KB" << endl;
...
int CFileLogDev::Open(void)
{
	...
	for (it = m_mapLogFile.begin(); it != m_mapLogFile.end(); ++it)
	{
		...
		{
			cout << "Log file:" << sFileFull << " open failed, ERRNO:" <<  errno << endl;
			continue;
		}
		...
		{
			cout << "Log file " << sFileFull << " get size failure" << endl;
		}
	}
	return 0;
}
...
int CFileLogDev::SwitchSize(LOG_FILE* pLogFile)
{
	...
	{
		cout << "SwitchSize log file:" << sFullName << " open failed, ERRNO:" <<  errno << endl;
		return -1;
	}
	...
	{
		cout << "Log file " << sFullName << " get size failure" << endl;
	}
...
int CFileLogDev::SwitchDate()
{
	...
	if (0 != MKDIR(m_sFilePath.c_str(),S_IRWXU | S_IRWXG | S_IRWXO))
	{
		cout << "Directory already exists!" << m_sFilePath << endl;
	}
	...
	for (it = m_mapLogFile.begin(); it != m_mapLogFile.end(); ++it)
	{
		...
		if(!(*it).second->pfLogFile)
		{
			cout << "SwitchDate log file:" << sFileFull << " open failed, ERRNO:" <<  errno << endl;
			continue;
		}
