...
int CLogger::Initial(CConfig * pCfg)
{
	...
	{
		//日志配置项错误
		cout << "Log configuration item error" << endl;
		return -1;
	}

	size_t nSize = vValues.size();
	cout << nSize << " log device" << endl;

	CLogDevice * pLogDev = 0;
	for (size_t i = 0; i < nSize; i++)
	{   // 各种日志设备
		cout << "log device" << vValues[i] << endl;
		
		CConfig * pTypeCfg = pCfg->GetProperties(vValues[i]);
		if (pTypeCfg == 0)
		{
			cout << "log device " << vValues[i] << " missing configuration" << endl;
			continue;
		}
		// 创建各种设备实例
		...
		//	cout << "目前不支持类型为" << sValue << "的日志设备" << endl;
			cout << "Currently it does not support the type of " << sValue << " log device" << endl;
			continue;
		}

		if (pLogDev->Initial(pTypeCfg) != 0)
		{
			cout << "Log device " << vValues[i] << " initialization failed" << endl;
			continue;
		}
	}
