...
//去除最外层的方括号
int stripBracket(string &sString) {
	if (sString.length()>1) {
		if ((sString[sString.length()-1]==']') && (sString[0]=='['))
...
//转义，在符号','、'\'、'['、']'、'{'、'}'前面增加'\'
std::string addTransStr(std::string sString)
{
	int size = sString.size();
	std::string sTemp;
	sTemp.reserve(sString.size()+50);
	sTemp = "";
	for (int i = 0 ;i <size; i ++)
	{
		if (sString.at(i)&0x80)
		{
			//中文字符
			sTemp += sString.at(i);
			sTemp += sString.at(++i);
...
//转义， 将'\,'、'\\'、'\['、'\]'、'\{'、'\}'转为','、'\'、'['、']'、'{'、'}'
std::string delTransStr(std::string sString)
{
	int size = sString.size();
	std::string sTemp;
	sTemp.reserve(sString.size());
	sTemp = "";
	for (int i = 0 ;i <size; i ++)
	{
		if (sString.at(i)&0x80)
		...
		{
		case '\\':
			//根据转义规则转义后不可能出现单个斜杠（'\\'）单独存在的情况，后面都会带需要转义的符号，因此解转义时不考虑单斜杠的情况
			i++;
			break;
		default:
			break;
...
//用于查找sString中出现的第一个非转义的cSeparator，支持中文
string::size_type findFirstOf(char cSeparator,const std::string &sString)
{
	string::size_type iPos = 0;
	//嵌套栈
	char cStack[20];
	int nStack = -1;
	for (iPos ; iPos<sString.size();iPos++)
	{
		if (sString.at(iPos)&0x80)
		{
			//中文字符
			iPos ++;
			continue;
		}
		if (sString.at(iPos)== cSeparator)
		{
			if (nStack == -1)
			{
				//没有转义
		...
		}else if (sString.at(iPos) == '\\')
		{
			if (nStack > -1 && cStack[nStack]=='\\')
			{
				-- nStack;
			}else
			{
...
//GTP报文格式，解析用cSeparator分割的字段，用于防止转义符'\\'，支持中文
std::vector< std::string > explodeGTP(char cSeparator,const std::string &sString)
{
	...
	vector< string > slPieces;
	string str;
	for (iEnd;iEnd<sString.size();iEnd++)
	{
		if (sString.at(iEnd)&0x80)
		{
			//中文字符
			iEnd ++;
			continue;
		}
		if (sString.at(iEnd)== cSeparator)
		{
			if (nStack == -1)
			{
				//嵌套栈为空，分隔符成立
				slPieces.insert(slPieces.end(),sString.substr(iBegin,iEnd-iBegin));
				//cout<<sString.substr(iBegin,iEnd-iBegin)<<endl;
				iBegin = iEnd + 1;
				continue;

			}else if (cStack[nStack] == '\\')
			{
				//为转义
				-- nStack;
			}
