...
public:	
	...
	int GetParameterVal(const char* pcParaName,char* pcVal,size_t sizeVal = 0);
	...
	int GetParameterVal(const std::string& sParaName,char* pcVal,size_t sizeVal = 0);
...
protected:
	...
	//获得从未被解析（既从未被GetParameterVal函数调用）的字段
	std::string AssembleBodyNotGet(std::string sParaSeperator,std::string sValSeperator);

private:
	typedef std::map<string,string> MAP_KEYVAL;
	MAP_KEYVAL m_mapParameters;
	MAP_KEYVAL m_mapBackParameters;
