...
std::string CPairPacket::AssembleBody(std::string sParaSeperator,std::string sValSeperator)
{
	...
	for(it=m_mapBackParameters.begin();it!=m_mapBackParameters.end();++it)
	{
		sEncode += it->first;
		sEncode += sValSeperator;
		sEncode += it->second;
...
std::string CPairPacket::AssembleBodyNotGet(std::string sParaSeperator,std::string sValSeperator)
{
	std::string sEncode = "";
	MAP_KEYVAL::iterator it;
...
int CPairPacket::GetParameterVal(const std::string& sParaName,std::string& sVal)
{
	...
	{
		sVal = it->second;
		//找到对应项之后从m_mapParameters中移动到m_mapBackParameters中，最后在m_mapParameters中就是剩下未解析的项
		m_mapBackParameters[s]=sVal;
		m_mapParameters.erase(it);
		nRtn = 0;
	}
	else
	{
		MAP_KEYVAL::iterator itBack = m_mapBackParameters.find(s);
		if (itBack != m_mapBackParameters.end())
		{
			sVal = itBack->second;
			nRtn = 0;
		}else
...
int CPairPacket::GetParameterVal(const std::string& sParaName,char* pcVal,size_t sizeVal)
{
	if (pcVal == 0)
		return -1;
	...
	if (nRtn == 0)
	{
		sTmp = strutils::delTransStr(sTmp);
		if (sizeVal <= 0)
		{
			//没有输入长度
		...
		{
			strncpy(pcVal,sTmp.c_str(),sizeVal);
			pcVal[sizeVal-1] = '\0';
		}
	}
...
int CPairPacket::GetParameterVal(const char* pcParaName,char* pcVal,size_t sizeVal)
{
	if (0 == pcParaName || 0 == pcVal)
		return -1;

	string sKey = pcParaName;
	return GetParameterVal(sKey,pcVal,sizeVal);
}
...
void CPairPacket::AddParameter(const char* pcParaName,const char* pcVal)
{
	...
	string sKey = pcParaName;
	string sVal = strutils::addTransStr(pcVal);
	AddParameter(sKey,sVal);
}
