...
CProtocolCommImp::~CProtocolCommImp()
{
	CRLog( E_DEBUG, "~CProtocolCommImp" );
}
...
int CTcpShortComm::Connect(const string& sIp, int nPort)
{
	...
	if (SOCKET_ERROR == connect( m_sSocket, (struct sockaddr*) &addrServer, sizeof(addrServer) ))
	{
		...
		CRLog(E_SYSINFO,"connect[%s:%d] failed",sIp.c_str(), nPort);
		return -1;
	}

	m_nState = E_CONNECTED;
	CRLog(E_SYSINFO, "connect[%s:%d] success", sIp.c_str(), nPort);
...
int CTcpShortComm::Connect()
{
	...
	{
		closesocket(m_sSocket);
		CRLog(E_DEBUG,"connect[%s:%d]failed",m_sIp.c_str(), m_nPort);
		return -1;
	}

	m_nState = E_CONNECTED;
	CRLog(E_DEBUG, "connect[%s:%d]success", m_sIp.c_str(), m_nPort);
