...
//
// $Id: //poco/1.7/UPnP/GENA/include/Poco/UPnP/GENA/EventModerationPolicy.h#1 $
//
// Library: UPnP/GENA
...
public:
	...
	void valueChanged(const T& newValue)
	{
		...
		{
			_value = newValue;
			if (_lastEvent.elapsed()/Poco::Clock::resolution() >= _maximumRate)
			{
				if (_pTimerTask)
				{
					_pTimerTask->cancel();
			...
			else if (!_pTimerTask)
			{
				...
				_pTimer->schedule(_pTimerTask, _lastEvent + _maximumRate*Poco::Clock::resolution());
			}
		}
	}
