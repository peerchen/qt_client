...
Layer3ForwardingProxy::Layer3ForwardingProxy(const Poco::RemotingNG::Identifiable::ObjectId& oid):
...
Layer3ForwardingProxy::~Layer3ForwardingProxy()
{
	if (_pEventListener)
	{
		try
...
void Layer3ForwardingProxy::getDefaultConnectionService(std::string& newDefaultConnectionService)
{
...
std::string Layer3ForwardingProxy::remoting__enableEvents(Poco::RemotingNG::Listener::Ptr pListener, bool enable)
{
	...
	if ((_pEventListener && !enable) || (!_pEventListener && enable))
	{
		...
		if (pEventListener)
		{
			if (enable)
			{
				...
				_pEventSubscriber = new Layer3ForwardingEventSubscriber(eventURI, this);
...
void Layer3ForwardingProxy::setDefaultConnectionService(const std::string& newDefaultConnectionService)
{
	remoting__staticInitBegin(REMOTING__NAMES);
...
const std::string Layer3ForwardingProxy::DEFAULT_NS("urn:schemas-upnp-org:service:Layer3Forwarding:1");
} // namespace InternetGateway1
} // namespace UPnPS
